...
if ([string]::IsNullOrWhiteSpace($ResultsDirectory)) {
    $ResultsDirectory = Join-Path $repositoryRoot "artifacts\validation\1.4.0.0"
}
...
function Assert-ManifestConsistency {
    ...
    if ($null -eq $packageIdentity -or
        $packageIdentity.Version -ne "1.4.0.0" -or
        $packageIdentity.Name -ne "mymiamo.net.MiaDock") {
        throw "Package identity or version is not the expected MiaDock 1.4.0.0 identity."
    }
