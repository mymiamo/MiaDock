...
if ([string]::IsNullOrWhiteSpace($ResultsDirectory)) {
    $ResultsDirectory = Join-Path $repositoryRoot "artifacts\validation\1.4.2.0"
}
...
function Assert-ManifestConsistency {
    ...
    if ($null -eq $packageIdentity -or
        $packageIdentity.Version -ne "1.4.2.0" -or
        $packageIdentity.Name -ne "mymiamo.net.MiaDock") {
        throw "Package identity or version is not the expected MiaDock 1.4.2.0 identity."
    }
...
function Assert-TestPackageContents {
    ...
    try {
        ...
        if ($null -eq $identity -or
            ...
            $identity.Version -ne "1.4.2.0" -or
            $null -eq $startupTask) {
