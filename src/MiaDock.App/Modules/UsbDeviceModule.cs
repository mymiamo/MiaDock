...
public sealed class UsbDeviceModule : IIslandModule, IAsyncDisposable
{
    public const string ModuleId = "usb-devices";
    private static readonly TimeSpan NotificationDuration = TimeSpan.FromSeconds(3);
    ...
    public ModuleDescriptor Descriptor { get; } = new(
        ...
        isPersistent: false,
        iconGlyph: "\uE88E",
        displayNameKey: "UsbDevice.ModuleName");
    ...
    private void OnDeviceChanged(object? sender, UsbDeviceChangedEventArgs args)
    {
        if (!ShouldMonitor)
        {
            return;
        ...
        void Publish()
        {
            ...
            var primary = _localization.Get(
                ...
                args.DisplayName);
            var secondary = args.IsConnected
                ? _localization.Get("UsbDevice.Connected")
                : _localization.Get("UsbDevice.Disconnected");
            var glyph = args.IsConnected ? "\uE88E" : "\uE74D";
